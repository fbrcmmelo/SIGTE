...
@Repository
public class TransporteDaoImpl extends AbstractDao<Transporte, Long> implements TransporteDao {

	@Override
	public PaginacaoUtil<Transporte> buscaParginada(int pagina) {
		...
		List<Transporte> transportes = getEntityManager()
				.createQuery("select t from Transporte t order by t.id asc", Transporte.class).setFirstResult(inicio)
		...
		return new PaginacaoUtil<>(tamanho, pagina, totalDePaginas, transportes);
	}
	...
	public long contador() {
		return getEntityManager().createQuery("select count(*) from Transporte", Long.class).getSingleResult();
	}

	@Override
	public List<Transporte> findTransporteByIdDay(Long id) {
		return createQuery("select t from Transporte t where t.diaSemana.id = ?1", id);
	}
}
